...
export async function fetchSchoolInfo(): Promise<SchoolRow | null> {
    const url = `https://open.neis.go.kr/hub/schoolInfo?key=${API_KEY}&Type=${TYPE}&pIndex=${pIndex}&pSize=${pSize}&SD_SCHUL_CODE=${SCHOOL_CODE}`;
    try {
        const json = await fetchWithTimeout(url, { timeoutMs: 3000, cacheByDay: true });
        return json?.schoolInfo?.[1]?.row?.[0] ?? null;
    } catch (e) {
